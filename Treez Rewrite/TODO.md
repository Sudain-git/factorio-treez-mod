...
- [ ] Barebones mod structure so the mod will load without error
- [ ] Individual file/control for tree seed logic
- [ ] individual file/control for tree queue logic
...
- [ ] `tree-seed` item (base-game fallback when Space Age is absent)
- [ ] `tree-seed` recipe (unlocked by Tree Growth research)
- [ ] `tree-seed-site` proxy entity — the placeable marker robots fulfil
...
- [ ] Seed placement rejection on hard tiles (concrete, stone, etc.)
- [ ] Seed placement rejection on natural resource patches (ore, oil, etc.)
- [ ] Seed placement rejection on Space Exploration scaffold tiles/surfaces
- [ ] Seed placement rejection when a player ghost is already on the tile
...
- [ ] `data-final-fixes.lua` — generates 200 hidden growth-stage prototypes per tree species
- [ ] Growth stage sprite scaling (sqrt curve, 5% floor)
- [ ] Growth stage collision box scaling (20%→100% of base tree)
...
- [ ] `try_dequeue_tree()` — pulls seeds from the queue when a slot opens
- [ ] `prune_growth_queue_for_surface()` — cleans up stale queue entries with no marker
- [ ] Stale-entry guard in `try_dequeue_tree` — detects seeds removed without a mined event
- [ ] Deconstruction-flag preservation — marked trees stay marked as they grow through stages
- [ ] Ghost conflict guard during growth — respects player-placed ghosts at a seed's tile
...
- [ ] `pick_species_at()` — samples nearby trees, picks species weighted by count and biome nativity
- [ ] Biome weight: native species (per `tile_buildability_rules`) get ×3 vs non-native ×1
- [ ] Fallback to tile's native species list when no nearby trees exist (cleared land)
- [ ] Dead/dry tree re-roll — sparse-looking species re-rolled once to reduce frequency
- [ ] Alien Biomes species support — builds `native_species` map from AB tile rules
- [ ] Vanilla-only fallback to `tree-01` when no biome data is available

---

## 4. Tree Seeder Area Tool

- [ ] `tree-seeder` selection-tool item
- [ ] `tree-seeder` shortcut button (toolbar)
- [ ] `tree-seeder` custom-input (keybind)
- [ ] Tree Seeder density GUI (slider, 10%–100%)
- [ ] `on_player_selected_area` handler — places seed-site ghosts at chosen density
- [ ] Hard tile / SE-blocked tile filtering in seeder area sweep
...
- [ ] `run_seed_ghost_collision_sweep()` — cancels unfulfillable seed ghosts blocked by foreign trees
- [ ] `run_foreign_tree_ghost_sweep()` — marks foreign trees that block any ghost for deconstruction
- [ ] `seed_ghost_sweep_tick()` — triggers sweeps once per day/night cycle at dusk, per surface
- [ ] `foreign_tree_sweep_enabled` setting toggle

---

## 10. Script-Raised Build Compatibility

- [ ] `script_raised_revive` handler — processes seeds/crystals placed by mods via `entity.revive()`
- [ ] `script_raised_built` handler — processes seeds/crystals placed by mods via `create_entity` + raise
...
- [ ] Alien Biomes — species map, tile upgrade/decay tables, dominant color sampling
- [ ] Space Exploration — blocked tile/surface detection, item refund on rejection
- [ ] Space Age — `plant` prototype support, `TREE_GROWTH_ENABLED` flag (growth disabled under SA)
...
- [ ] AAI Industry — adjusted tech prerequisite for Sapling Growth 1
- [ ] `is_se_blocked_tile` / `is_se_blocked_surface` helpers
...
- [ ] `/treez` — prints queue snapshots, handle_site counters, config state
- [ ] `/treez reset` — clears debug counters
...
- [ ] - Review individual art graphics for seeds
- [ ] - Review individual art graphics for nitrates
- [ ] - Review mapping between art graphics and how they are used
